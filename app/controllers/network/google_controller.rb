...
class Network::GoogleController < ApplicationController
  ...
  def refresh
    if current_user && current_user.authentications.count > 0
      ...
      #if count_messages > 0
      if mailbox.last_update
        latest_message_datetime = mailbox.last_update #mailbox.messages[count_messages-1].message_datetime
        puts "Latest message datetime: #{mailbox.last_update}"
